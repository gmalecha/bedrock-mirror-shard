...
Proof.
  ...
  { simpl. rewrite heq_star_emp_r. reflexivity. }
...
Proof.
  ...
  { replace (offset + 4 * 0) with offset by omega.
    simpl minus.
    unfold allocated at 1. rewrite heq_star_emp_l. reflexivity. }
  { replace (S len' - S len') with 0 by omega. 
    unfold allocated at 3. rewrite heq_star_emp_r. reflexivity. }
  { subst.
    replace (offset + 4 * S len') with ((4 + offset) + 4 * len') by omega.
