...
Require Import Bedrock.CancelTacIL.

Notation "'hprop'" := Bedrock.SepIL.ST.hprop.
...
Fixpoint chain {ts} (ls : list (Expr.expr ts)) {struct ls} : list (SEP.sexpr ts) :=
  match ls with
    | nil => nil
    | a :: b =>
      match b with
        | nil => nil
        | c :: d => (SEP.Func 0 (a :: c :: nil)) :: (chain b)
      end
  end%Sep.

Definition chainr {ts} (ls : list (Expr.expr ts)) : SEP.sexpr ts :=
  List.fold_right (@SEP.Star _) (@SEP.Emp _) (chain ls).

Definition chainl {ts} (ls : list (Expr.expr ts)) : SEP.sexpr ts :=
  List.fold_left (@SEP.Star _) (chain ls) (@SEP.Emp _).
...
Check CancelTacIL.nexistsSubst.
...
Definition cancel (himp : hprop -> hprop -> Prop) (emp : hprop) (star : hprop -> hprop -> hprop)
  ...
  match CANCEL_TAC.canceller tpreds (TacPackIL.UNF.Forward hints) (TacPackIL.UNF.Backward hints) prover bound bound (Expr.typeof_env uvars) hyps lhs rhs with
                  | None =>
                  ...
                  forallEach new_vars (fun nvs : env ts =>
                    let var_env := nvs in
                      ...
                      (@CancelTacIL.nexistsSubst not _ funcs var_env subst 0
                        (map
